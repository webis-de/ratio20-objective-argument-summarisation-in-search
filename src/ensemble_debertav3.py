...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(os.path.join("/bigwork/nhwpziet/appropriateness-style-transfer/data/models/binary-debertav3-conservative-no-issue/", f"fold{args.repeat}/0", args.checkpoint))
    ...
    for i in range(args.model_count):
        trainer = Trainer(
                ...
                tokenizer=tokenizer,
                model=AutoModelForSequenceClassification.from_pretrained(os.path.join("/bigwork/nhwpziet/appropriateness-style-transfer/data/models/binary-debertav3-conservative-no-issue/", f"fold{args.repeat}/{i}", args.checkpoint))
        )
    ...
    # with dataset name
    with open(os.path.join(args.output, f"ensemble_predictions_{args.dataset_name}.txt"), "w") as f:
